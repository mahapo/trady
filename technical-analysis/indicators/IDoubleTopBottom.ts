...
export class IDoubleTopBottom {
  static name = "DoubleTopBottom";

  // Static method to generate trading signals based on candlestick patterns
  static signal(candles: OHLCV[]): number {
    const patterns = IDoubleTopBottom.detectPatterns(candles);
    ...
    if (latestPattern.pattern === "Head and Shoulders Top" || latestPattern.pattern === "Double Top") {
      return -1;  // Sell signal for bearish patterns
    } else if (latestPattern.pattern === "Head and Shoulders Bottom" || latestPattern.pattern === "Double Bottom") {
